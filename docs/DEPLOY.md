...
- Кыргызские названия районов сгенерированы по общим знаниям, не сверены
  носителем языка
- Расчёт NDVI по границе конкретного участка (`geointel/scripts/compute_field_metrics.py`)
  написан и подключён к API (`GET /api/fields/{id}/series`) и фронтенду
  («Мои участки» → «Показать»), но ни разу не выполнялся вживую — блокирует
  та же проблема с правами Earth Engine (Фаза 1). После починки IAM запустить
  вручную (`uv run python -m geointel.scripts.compute_field_metrics`) и
  добавить вторым Cloud Run Job'ом рядом с `geointel-batch` в Фазе 7, на
  тот же дневной Scheduler.
